...
import {
    getAllBaskets, getProductByIdFromBaskets, updateBaskets as updateBasketsAction
...
function ProductCard({borderPosition = 'outer', product}) {
  ...
  const dispatch= useDispatch();
    ...
    return (
        <div
            className={`${Classes.card} ${borderPosition === 'outer' ? Classes.productBorder : ''}`}>
            ...
            <div className={Classes.cardContent}>
                ...
                <div className={Classes.priceContainer}>
                    <span
                        className={`${Classes.sellingPrice} ${product.off !== 0 && Classes.discounted}`}>{product.selling_price.toLocaleString()} تومان</span>
                    {
                        product.off !== 0 &&
